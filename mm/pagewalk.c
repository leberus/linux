				      ...
				      pt_type_flags_t flags)
{
	...
	unsigned long next_addr, curr_addr = ptw->next_addr ? ptw->next_addr : addr;

	if (WARN_ON_ONCE(curr_addr < vma->vm_start || curr_addr >= vma->vm_end))
		return ret_type;

	/* We cannot do much for gate_vma */
	if (!ptw->mm)
		return ret_type;
...
found:
	/* Fill in ptw struct before returning */
	ptw->ptl = ptl;
	ptw->next_addr = next_addr;
