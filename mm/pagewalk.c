...
pmd_table:
	...
	if (IS_ENABLED(CONFIG_PGTABLE_HAS_HUGE_LEAVES) &&
		...
		if (pmd_none(pmd)) {
			 ...
			 */
			folio = vm_normal_folio_pmd(vma, curr_addr, pmd);
			if (folio) {
				if (!(flags & PT_TYPE_FOLIO))
					goto keep_walking;
				ret_type = PTW_FOLIO;
			}
			if (folio && folio_size(folio) > entry_size) {
				/* We can batch */
				int max_nr = folio_size(folio) / entry_size;

				nr_batched = folio_pmd_batch(folio, curr_addr,
							     pmdp, pmd, max_nr,
							     0, &writable,
							     &young, &dirty);
		...
		if (folio && nr_batched == 1) {
			/* If we batched, we already have the values */
			dirty = !!pmd_dirty(pmd);
			young = !!pmd_young(pmd);
...
pte_table:
	...
	if (pte_none(pte)) {
		...
		if (folio && folio_test_large(folio)) {
		        /* We can batch */
			unsigned long end_addr = pmd_addr_end(curr_addr, vma->vm_end);
			int max_nr = (end_addr - curr_addr) >> PAGE_SHIFT;

			nr_batched = folio_pte_batch(folio, ptep, pte, max_nr);
			dirty = !!pte_dirty(pte);
			young = !!pte_young(pte);
			writable = !!pte_write(pte);
		} else if (!folio && (is_zero_pfn(pte_pfn(pte)) ||
			   vma->vm_flags & VM_PFNMAP)) {
				if (!(flags & PT_TYPE_PFN))
					goto not_found;
				ret_type = PTW_PFN;
		...
		if (softleaf_is_marker(entry)) {
			...
			int max_nr = (end - curr_addr) / entry_size;
...
found:
	...
	if (folio) {
		ptw->folio = folio;
		ptw->page = folio_page(folio, 0) + 
				     ((curr_addr & (entry_size - 1)) >> PAGE_SHIFT);
	}
	return ret_type;
}
