...
static bool memmap_on_memory_enabled __ro_after_init = false;
			       ...
			       unsigned long buddy_start_pfn)
{
	...
	/*
	 * When using memmap_on_memory, the range might be unaligned as
	 * the first pfns are used for vmemmap pages.
	 * Let us align it in case we need to.
	 */
	if (pfn & ((1 << order) - 1)) {
		while (pfn & ((1 << order) - 1))
			order--;
...
static int __ref try_remove_memory(int nid, u64 start, u64 size)
{
	 ...
	 * blocks we try to remove and see if any has vmemmap pages.
	...
	if (memmap_on_memory_enabled) {
		...
		if (nr_vmemmap_pages) {
			if (size != memory_block_size_bytes()) {
				pr_warn("Refuse to remove %#llx - %#llx. When"
					 "using memmap_on_memory,"
					 "{add,remove}_memory granularity must"
					 "be the same\n", start, start + size);
				return -EINVAL;
			} else {
				/*
