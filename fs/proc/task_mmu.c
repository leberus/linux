				  ...
				  unsigned long start)
{
	...
	while (type != PTW_DONE) {
		...
		case PTW_SWAP:
			...
			smaps_account(mss, ptw.page, false, ptw.young, ptw.dirty,
				      locked, ptw.present);
			break;
		default:
			/* Ooops */
...
static int show_numa_map_lab(struct seq_file *m, void *v)
{
	...
	do {
		ret = pt_range_walk_start(vma->vm_start, vma->vm_end, &ptw);

		if (!ptw.folio || !ret)
			continue;

		if (!ptw.page || PageReserved(ptw.page))
			continue;
