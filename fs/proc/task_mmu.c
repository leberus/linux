		...
		struct mm_walk *walk)
{
	...
	smaps_account(mss, page, false, young, dirty, locked, present, 0);
}
	...
	while (type != PTW_DONE) {
		...
		if (account && ptw.folio)
			smaps_account(mss, ptw.page, compound, ptw.young,
				      ptw.dirty, locked, ptw.present, ptw.nr_entries);
...
static void pagemap_scan_walk(struct vm_area_struct *vma, struct pagemap_scan_private *p,
			      unsigned long addr)
{
	struct pt_range_walk ptw = {
		.mm = vma->vm_mm
...
static long do_pagemap_scan_lab(struct mm_struct *mm, unsigned long uarg)
{
	...
	for (walk_start = p.arg.start; walk_start < p.arg.end;
		...
		do {
			...
			if (vma) {
				pagemap_scan_walk(vma, &p, walk_start);
...
static void pagemap_read_walk_range(struct vm_area_struct *vma, unsigned long start,
				    struct pagemapread *pm)
{
	int err;
	struct pt_range_walk ptw = {
	...
	while (type != PTW_DONE) {
		...
		case PTW_PUD_LEVEL:
			...
			if (pud_present(ptw.pud)) {
				folio = page_folio(pud_page(ptw.pud));
				flags |= PM_PRESENT;
		...
		if (ptw.level == PTW_PTE_LEVEL) {
			...
			for (; start < end; ptep++, start += PAGE_SIZE) {
				...
				err = add_to_pagemap(&pme, pm);
				ptw.next_addr = ptw.curr_addr + PAGE_SIZE;
				if (err)
					break;
			}
		} else {
			    ...
			    size_t count, loff_t *ppos)
{
	...
	while (count && (start_vaddr < end_vaddr)) {
		...
		do {
			...
			if (vma) {
				pagemap_read_walk_range(vma, start_vaddr, &pm);
